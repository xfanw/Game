...
{
    public class SelectBatchMan : ManagerBase
    {
        private static SelectBatchMan _batchManager;
        private SDLinkedList Batches = new SDLinkedList();
        private Batch compareBatch = new Batch();

        // Singleton Pattern
        private SelectBatchMan(int t, int d) : base(t, d) { }

        private static SelectBatchMan GetInstance(int t = 4, int d = 1)
        {
            if (_batchManager == null)
            {
                _batchManager = new SelectBatchMan(t, d);
            }
